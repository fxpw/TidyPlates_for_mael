...
local function IsTotem2(name)
	if name then
		return (Totem_InfoTable2[name] ~= nil)
	end
end
...
local function UpdateTotemIconWidget(self, unit)
	...
	if icon then
		...
		self:Show()
		if self.Icon:GetWidth() ~= TidyPlatesHubDamageVariables.WidgetsTotemIconSize then
		self.Icon:SetSize(TidyPlatesHubDamageVariables.WidgetsTotemIconSize,TidyPlatesHubDamageVariables.WidgetsTotemIconSize)
		end
	else
		self:Hide()
	end
...
local function CreateTotemIconWidget(parent)
	...
	-- parent:Show()
	frame:SetSize(TidyPlatesHubDamageVariables.WidgetsTotemIconSize,TidyPlatesHubDamageVariables.WidgetsTotemIconSize)
	...
	frame.Icon:SetPoint("TOP", frame,0,30)
	frame.Icon:SetTexCoord(.07, 1 - .07, .07, 1 - .07) -- obj:SetTexCoord(left,right,top,bottom)
	frame.Icon:SetSize(TidyPlatesHubDamageVariables.WidgetsTotemIconSize,TidyPlatesHubDamageVariables.WidgetsTotemIconSize)
	...
	if IsTotem2(parent.unit.name) then
		print(parent.unit.name)
		parent:Hide()
	end
